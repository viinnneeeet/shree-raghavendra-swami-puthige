    ...
    <nav className="fixed top-0 left-0 right-0 z-50 bg-background/95 backdrop-blur-sm border-b border-temple-gold/20 shadow-temple">
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between h-16">
          <Link
            to="/"
            className="text-2xl font-bold bg-gradient-to-r from-temple-gold to-temple-bronze bg-clip-text text-transparent hover:scale-105 transition-transform duration-300">
            {TEMPLE_NAME}
          </Link>
              ...
              <Link
                ...
                className={classNames(
                  'px-3 py-2 rounded-md text-sm font-medium transition-all duration-300 hover:scale-105',
                  {
                    ...
                    'text-foreground hover:text-temple-gold': location.pathname !== item.href,
                  }
                )}>
                {item.label}
              </Link>
            ...
            {NAVIGATION_ITEMS.map((item, index) => (
              <Link
                ...
                className={classNames(
                  ...
                  {
                    'text-temple-gold bg-temple-gold/10': location.pathname === item.href,
                    'text-foreground hover:text-temple-gold hover:bg-temple-gold/5': location.pathname !== item.href,
                  },
