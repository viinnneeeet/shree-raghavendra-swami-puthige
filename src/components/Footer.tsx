      ...
      <div className="container mx-auto px-4 sm:px-6">
        <div className="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-4 gap-6 sm:gap-8">
          ...
          <div className="md:col-span-1 lg:col-span-2">
            <h3 className="lg:text-xl md:text-4xl font-bold bg-gradient-to-r from-temple-gold to-temple-sunset bg-clip-text text-transparent mb-3 sm:mb-4">
              About {TEMPLE_NAME}
            </h3>
            <p className="text-white/85 md:text-2xl mb-4 sm:mb-6 lg:text-sm sm:text-base leading-relaxed">
          ...
          <div className="mt-4 sm:mt-0">
            <h4 className="lg:text-lg md:text-4xl font-semibold mb-3 sm:mb-4 text-temple-gold">
              Quick Links
            </h4>
            <div className="space-y-2 sm:space-y-3">
              {QUICK_LINKS?.map((link) => (
                <Link
                  key={link?.name}
                  to={link?.url}
                  className="block text-white/80 hover:text-temple-gold transition-all duration-300 hover:translate-x-1 lg:text-sm md:text-2xl sm:text-base py-1">
                  {link?.name}
                </Link>
              ))}
            </div>
